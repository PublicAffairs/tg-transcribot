...
- **Instant & Efficient**: Unlike traditional containers that suffer from slow cold starts and waste resources by idling, your serverless bot wakes up instantly to process messages and scales down to zero immediately after. No idle waste!
...
- **Superior Quality**: Better understands context, jargon, and accents.
...
> For any platform, you must configure the required [configuration variables](#️-configuration-variables) (`TELEGRAM_BOT_TOKEN`, `WHISPER_API_KEY`) during deployment.
...
Once your serverless deployment is live on any of the platforms above and has a public URL (e.g. `https://your-bot.example.com`), register the webhook URL with Telegram:
